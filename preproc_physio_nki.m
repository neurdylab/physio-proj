...
% parameters 
% ------------------------------------------ %
% list of files from subjects with good physio
good = '/home/songrw/neurdy/tsvfiles/good.txt';
A = readlines(good, 'WhitespaceRule','trim'); 
[rows, cols] = size(A); 
for loopVar = 180:499
    preprocess_files(A(loopVar));
end

function preprocess_files(sub_id)

    fs_phys = 62.5; % nki sampling rate physio
    TR = 1.4; % fMRI interval between volumes
    nframes = 404; % double check this
    
    % init subject id
    %id = '00065487';
    
    % save director;
    my_path = '/home/songrw/neurdy/preproc/';
    ...
    %tsv_path = append('/home/songrw/neurdy/tsvfiles/', id);
    disp(tsv_path)
    phys = load(tsv_path);
    [ro, co] = size(phys); 
    % ------------------------------------------ %
    ...
    % save_dir = '/Users/rachelyang/Desktop/NeurdyLab/2022Spring/Analysis/';
    % if ~isempty(save_dir)
    %     if ~exist(save_dir,'dir')
    %         mkdir(save_dir);
    %     end
    %     save([save_dir,'/','test','_physOUT.mat'],'OUT_p','TABLE','REGS');
    % end
    
    % whole file name & 3 figures
end 
